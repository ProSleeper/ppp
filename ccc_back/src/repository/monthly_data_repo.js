...
const save = (data_obj) => {
    return new Promise((resolve, reject) => {
        ...
        for (let index = 0; index < 23; index++) {
            if (update_data[`day${index}_price`] === null) {
                delete update_data[`day${index}_price`];
            }
        }
...
const findAll = () => {
    return new Promise((resolve, reject) => {
        connection.query("select * from monthly_data", function (error, rows, fields) {
            if (error) reject(error);
            resolve(rows);
        });
    });
...
const findByUrl = (url) => {
    return new Promise((resolve, reject) => {
        connection.query(`select * from monthly_data where url = '${url}'`, function (error, rows, fields) {
            if (error) reject(error);
            resolve(rows);
        });
    });
