...
const save = (data_obj) => {
    return new Promise((resolve, reject) => {
        ...
        connection.query(
            `insert into deleted_url_data set ? on duplicate key update ?`,
...
const remove = (data_obj) => {
    return new Promise((resolve, reject) => {
        connection.query(
            `delete from deleted_url_data where ?`,
...
const findAll = () => {
    return new Promise((resolve, reject) => {
        connection.query("select * from deleted_url_data", function (error, rows, fields) {
            if (error) reject(error);
            resolve(rows);
        });
    });
...
const findByUrl = (url) => {
    return new Promise((resolve, reject) => {
        connection.query(`select * from deleted_url_data where url = '${url}'`, function (error, rows, fields) {
            if (error) reject(error);
            resolve(rows);
        });
    });
